    ...
    <section className='pt-28 text-center px-6 bg-white'>
      ...
      <ul className='mt-16'>
        {products.map(({ bg, descr, price }) => (
          <li className='mb-12'>
            <div
              className={`${bg} h-[25rem] bg-cover bg-[center_bottom_-7rem] bg-no-repeat p-5 flex items-end cursor-pointer group hover:shadow-[0_0_3px_#00000060]`}
            >
      ...
      <button className='mx-auto tracking-wide text-sm gap-4 bg-zinc-900 text-white flex-x px-7 py-4 group duration-300'>
        ...
        <Icon className='text-2xl group-hover:translate-x-2 duration-300' icon='bi:arrow-right' /> </button>
    </section>
  );
}
